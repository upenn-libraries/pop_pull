...
def usage():
    print "Usage: %s IDFILE OUTFILE" % (os.path.dirname(__file__), )
...
def write_info(ids, key, outfile):
    ...
    for photo_id in ids:
        try:
            s = json.dumps(get_info(photo_id,key))
            s = bidi_re.sub('', s)
            f.write(s)
            f.write(',')
...
try:
    ...
    print "Please provide an OUTFILE"
    usage()
    sys.exit(1)

ids = [ phid.strip() for phid in open(idfile) ]

write_info(ids, key, outfile)
